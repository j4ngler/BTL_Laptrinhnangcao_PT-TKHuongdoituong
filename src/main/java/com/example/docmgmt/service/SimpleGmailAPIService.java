...
/**
 * Simple Gmail API Service for 10+ Gmail accounts
 * Sử dụng Gmail API với OAuth 2.0
 */
public class SimpleGmailAPIService {
    ...
    public SimpleGmailAPIService(String email, String credentialsPath, 
    ...
    public int fetchEmails(String query) throws Exception {
        ...
        System.out.println("Processed " + processedCount + " emails from " + email);
    ...
    private Document createDocumentFromEmail(String subject, String from) {
        try {
            ...
            String fileId = saveMockFile(subject);
     ...
     * Lưu mock file vào GridFS
     */
    private String saveMockFile(String subject) {
        try {
    ...
    public boolean testConnection() {
        try {
            ...
            System.out.println("Testing connection to " + email + "...");
            Thread.sleep(1000); // Simulate connection test
            System.out.println("Connection test successful for " + email);
            return true;
        } catch (Exception e) {
            System.err.println("Connection test failed for " + email + ": " + e.getMessage());
            return false;
        }
    }
